...
# 对于Data导入格式进行更改，把MambaStock运用在比特币上，后续需要根据不同比特币的数据format进行调整。
# 如果需要使用原始MambaStock对于股票进行预测，就comment掉 Work with Bitcoin 部分
# 然后使用Work with Stock部分即可。两种都可以跑通，数据都已经上传了。
...
def evaluation_metric(y_test,y_hat):
    ...
    print('%.4f %.4f %.4f %.4f %.4f' % (MSE,RMSE,MAE,MAPE,R2))
...
data['trade_date'] = pd.to_datetime(data['trade_date'].str.slice(0,10))
data['ratechg']=(data['close']-data['open'])/data['open']
ratechg=data['ratechg'].values
close = data.pop('close').values
dat = data.iloc[:,2:].values
trainX, testX = dat[:-args.n_test, :], dat[-args.n_test:, :]
trainy = ratechg[:-args.n_test]
